...
class ProxyScraper {
    ...
    classifyProxySpeed(responseTime) {
        if (responseTime <= 2000) return 'FAST';      // ≤ 2 detik
        if (responseTime <= 5000) return 'MEDIUM';    // ≤ 5 detik  
        if (responseTime <= 10000) return 'SLOW';     // ≤ 10 detik
        return 'VERY_SLOW';                           // > 10 detik
    }
    ...
    async getWorkingProxies(maxProxies = 15) {
        console.log('🎯 Mengambil proxy (HTTP/HTTPS/SOCKS) dengan adaptive timeout...');
        ...
        console.log(`🔍 Adaptive ping test ${Math.min(allProxies.length, 25)} proxies...`);
        ...
        const fastProxies = workingProxies.filter(p => {
            const data = this.activeProxies.get(p);
            return data && data.speedCategory === 'FAST';
        });
        const mediumProxies = workingProxies.filter(p => {
            const data = this.activeProxies.get(p);
            return data && data.speedCategory === 'MEDIUM';
        });
        const slowProxies = workingProxies.filter(p => {
            const data = this.activeProxies.get(p);
            return data && data.speedCategory === 'SLOW';
        });
        const verySlowProxies = workingProxies.filter(p => {
            const data = this.activeProxies.get(p);
            return data && data.speedCategory === 'VERY_SLOW';
        });
        
        console.log(`📊 Klasifikasi Kecepatan: FAST: ${fastProxies.length}, MEDIUM: ${mediumProxies.length}, SLOW: ${slowProxies.length}, VERY_SLOW: ${verySlowProxies.length}`);
