...
class TrafficGenerator {
  ...
  async startNewSession(config) {
    ...
    this.log(sessionId, 'SESSION_INIT', 'Initializing new session dengan multi-protocol proxy testing...');
    
    let proxyList = [];
    
    if (config.proxySource === 'auto') {
      this.log(sessionId, 'PROXY_AUTO', 'Mengambil proxy (HTTP/HTTPS/SOCKS) dengan basic ping test...');
      ...
      try {
        ...
        if (proxyList.length > 0) {
          ...
          this.log(sessionId, 'PROXY_AUTO_SUCCESS', 
            `Berhasil mendapatkan ${proxyList.length} proxy aktif`);
          
          proxyList.forEach((proxy, index) => {
            const proxyData = this.proxyScraper.activeProxies.get(proxy);
            const info = proxyData ? `(IP: ${proxyData.ip}, Tipe: ${proxyData.type.toUpperCase()}, Kualitas: ${proxyData.quality})` : '';
      ...
      this.log(sessionId, 'PROXY_MANUAL', `Testing ${config.proxyList.length} manual proxies (HTTP/HTTPS/SOCKS)...`);
      ...
      if (workingProxies.length > 0) {
        ...
        this.log(sessionId, 'PROXY_MANUAL_SUCCESS', 
          `${workingProxies.length}/${config.proxyList.length} proxy manual berhasil`);
        
        const httpCount = workingProxies.filter(p => this.proxyScraper.detectProxyType(p) === 'http').length;
        const httpsCount = workingProxies.filter(p => this.proxyScraper.detectProxyType(p) === 'https').length;
        const socksCount = workingProxies.filter(p => this.proxyScraper.detectProxyType(p).includes('socks')).length;
        
        this.log(sessionId, 'PROXY_TYPE_SUMMARY', `HTTP: ${httpCount}, HTTPS: ${httpsCount}, SOCKS: ${socksCount}`);
      } else {
    ...
    this.log(sessionId, 'SESSION_STARTED', 
      ...
      ` menggunakan ${proxyList.length} proxy aktif (Multi-protocol)` +
  ...
  async fallbackToBackupProxies(config, sessionId) {
    this.log(sessionId, 'PROXY_FALLBACK', 'Menggunakan backup proxies (Multi-protocol)...');
  ...
  async executeSessionWithRetry(sessionId, config, retryCount = 0) {
    ...
    try {
      ...
      if (isProxyError && retryCount < maxRetries) {
        ...
        if (newProxy) {
          this.log(sessionId, 'NEW_PROXY', `Menggunakan proxy baru: ${newProxy} (${this.proxyScraper.detectProxyType(newProxy).toUpperCase()})`);
  ...
  async executeSession(sessionId, config) {
    ...
    try {
      ...
      this.log(sessionId, 'PROXY_SELECTED', `Menggunakan proxy: ${currentProxy} (${proxyType.toUpperCase()})`);
      ...
      this.log(sessionId, 'STEP_1_COMPLETE', `Browser launched dengan ${config.deviceType} user agent dan ${proxyType.toUpperCase()} proxy`);

      this.log(sessionId, 'STEP_2', `Navigating ke: ${config.targetUrl} dengan ${proxyType.toUpperCase()} proxy`);
      ...
      try {
        ...
        this.log(sessionId, 'STEP_2_COMPLETE', `Berhasil navigasi ke target URL dengan ${proxyType.toUpperCase()} proxy`);
