...
class TrafficGenerator {
  ...
  async startNewSession(config) {
    ...
    this.log(sessionId, 'SESSION_INIT', 'Initializing new session dengan basic proxy testing...');
    
    let proxyList = [];
    
    if (config.proxySource === 'auto') {
      this.log(sessionId, 'PROXY_AUTO', 'Mengambil proxy dengan basic ping test...');
      ...
      try {
        ...
        if (proxyList.length > 0) {
          ...
          proxyList.forEach((proxy, index) => {
            ...
            const info = proxyData ? `(IP: ${proxyData.ip}, Kualitas: ${proxyData.quality})` : '';
      ...
      // Untuk manual, kita test satu per satu dengan basic ping test
  ...
  async fallbackToBackupProxies(config, sessionId) {
    ...
    if (config.backupProxies && config.backupProxies.length > 0) {
      // Test backup proxies dengan basic ping test
  ...
  async launchBrowser(config, proxyUrl) {
    ...
    // PERUBAHAN PENTING: headless: "new" untuk menghindari deprecation warning
    const launchOptions = {
      headless: "new", // Ganti dari true ke "new"
      args: args,
