...
class TrafficGenerator {
  ...
  async startNewSession(config) {
    ...
    this.log(sessionId, 'SESSION_INIT', 'Initializing new session dengan native proxy solution...');
    ...
    this.log(sessionId, 'SESSION_STARTED', 
      ...
      ` menggunakan ${proxyList.length} proxy aktif (Native Proxy Solution)` +
    ...
    try {
      ...
      if (isProxyError && retryCount < maxRetries) {
        this.log(sessionId, 'PROXY_RETRY', 
          `Proxy error, mencoba session baru dengan proxy lain... (${retryCount + 1}/${maxRetries})`);
  ...
  async executeSession(sessionId, config) {
    ...
    try {
      ...
      // 🆕 LAUNCH BROWSER DENGAN PROXY NATIVE
      this.log(sessionId, 'STEP_1', `Launching browser dengan native proxy ${proxyTimeout/1000}s timeout...`);
      ...
      try {
        ...
        this.log(sessionId, 'SESSION_COMPLETED', 'Semua steps completed successfully dengan native proxy');
  ...
  // 🆕 METHOD LAUNCH BROWSER DENGAN PROXY NATIVE
  ...
  async launchBrowser(config, proxyUrl) {
    ...
    // 🆕 TAMBAHKAN PROXY SERVER KE ARGS
    ...
    const proxyType = this.proxyScraper.detectProxyType(proxyUrl);
    console.log(`Launching browser dengan ${proxyType.toUpperCase()} proxy:`, browserProxy);
