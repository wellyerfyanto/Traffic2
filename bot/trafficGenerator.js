...
class TrafficGenerator {
  ...
  // 🆕 FORMAT PROXY UNTUK PUPPETEER-PAGE-PROXY
  formatProxyForPageProxy(proxyUrl) {
    ...
    // Default ke http jika tidak ada protocol
  ...
  async startNewSession(config) {
    ...
    this.log(sessionId, 'SESSION_INIT', 'Initializing new session dengan puppeteer-page-proxy system...');
    
    let proxyList = [];
    
    if (config.proxySource === 'auto') {
      this.log(sessionId, 'PROXY_AUTO', 'Mengambil proxy dengan extended timeout system...');
      ...
      try {
        ...
        if (proxyList.length > 0) {
          ...
          this.log(sessionId, 'PROXY_AUTO_SUCCESS', 
            `Berhasil mendapatkan ${proxyList.length} proxy aktif dengan extended timeout`);
      ...
      this.log(sessionId, 'PROXY_MANUAL', `Testing ${config.proxyList.length} manual proxies dengan extended timeout...`);
      ...
      if (workingProxies.length > 0) {
        ...
        this.log(sessionId, 'PROXY_MANUAL_SUCCESS', 
          `${workingProxies.length}/${config.proxyList.length} proxy manual berhasil dengan extended timeout`);
        ...
        this.log(sessionId, 'PROXY_SPEED_SUMMARY', `ULTRA_FAST: ${ultraFast}, FAST: ${fast}, MEDIUM: ${medium}, SLOW: ${slow}, VERY_SLOW: ${verySlow}, EXTREMELY_SLOW: ${extremelySlow}`);
      } else {
    ...
    this.log(sessionId, 'SESSION_STARTED', 
      ...
      ` menggunakan ${proxyList.length} proxy aktif (Puppeteer-Page-Proxy)` +
  ...
  async fallbackToBackupProxies(config, sessionId) {
    this.log(sessionId, 'PROXY_FALLBACK', 'Menggunakan backup proxies dengan extended timeout...');
  ...
  async executeSessionWithRetry(sessionId, config, retryCount = 0) {
    const maxRetries = 3; // Tambah max retries
    ...
    try {
      ...
      if (isProxyError && retryCount < maxRetries) {
        ...
        if (newProxy) {
          ...
          await new Promise(resolve => setTimeout(resolve, 3000)); // Kurangi delay
  ...
  async executeSession(sessionId, config) {
    ...
    let proxyTimeout = 90000; // Default timeout lebih panjang
    let page = null;
    
    try {
      // 🆕 LAUNCH BROWSER TANPA PROXY DI ARGS
      this.log(sessionId, 'STEP_1', 'Launching browser TANPA proxy settings...');
      browser = await this.launchBrowserWithoutProxy(config);
      
      page = await browser.newPage();
      
      // Dapatkan proxy terbaik
      ...
      const stepMultiplier = proxyInfo.proxyData.stepMultiplier || 1.5;
      ...
      this.log(sessionId, 'PROXY_SELECTED', 
        ...
        `Step Multiplier: ${stepMultiplier}x`);

      // 🔥 SET PROXY MENGGUNAKAN PUPPETEER-PAGE-PROXY
      const formattedProxy = this.formatProxyForPageProxy(currentProxy);
      await useProxy(page, formattedProxy);
      this.log(sessionId, 'PROXY_APPLIED', `Proxy berhasil diaplikasikan menggunakan puppeteer-page-proxy`);
      ...
      this.log(sessionId, 'STEP_2', 
        `Navigating ke: ${config.targetUrl} dengan extended timeout ${proxyTimeout/1000} detik...`);
      ...
      try {
        ...
        this.log(sessionId, 'STEP_2_COMPLETE', 
          `Berhasil navigasi ke target URL dengan ${proxyType.toUpperCase()} proxy dan extended timeout`);

        await this.executeAllSteps(page, sessionId, config, stepMultiplier);

        this.log(sessionId, 'SESSION_COMPLETED', 'Semua steps completed successfully dengan puppeteer-page-proxy');

      } catch (navError) {
        this.log(sessionId, 'NAVIGATION_ERROR', 
          `Navigation gagal dengan extended timeout ${proxyTimeout/1000}s: ${navError.message}`);
        if (currentProxy) {
          this.proxyScraper.markProxyAsFailed(currentProxy);
        }
        
        // 🆕 COBA PROXY LAIN TANPA RESTART BROWSER
        if (page && this.shouldRetryWithNewProxy(navError)) {
          const newProxyInfo = this.proxyScraper.getProxyWithOptimalTimeout();
          if (newProxyInfo && newProxyInfo.proxyUrl !== currentProxy) {
            this.log(sessionId, 'QUICK_SWITCH', `Quick switch ke proxy baru: ${newProxyInfo.proxyUrl}`);
            const newFormattedProxy = this.formatProxyForPageProxy(newProxyInfo.proxyUrl);
            await useProxy(page, newFormattedProxy);
            
            // Coba navigasi lagi dengan proxy baru
            await page.goto(config.targetUrl, { 
              waitUntil: 'domcontentloaded',
              timeout: newProxyInfo.proxyData.optimalTimeout
            });
            this.log(sessionId, 'QUICK_SWITCH_SUCCESS', 'Berhasil navigasi dengan proxy baru');
          }
        } else {
          throw navError;
        }
  ...
  // 🆕 METHOD UNTUK LAUNCH BROWSER TANPA PROXY
  async launchBrowserWithoutProxy(config) {
    const args = [
      '--no-sandbox',
      ...
      // 🚫 TIDAK ADA --proxy-server DI SINI
    ];
    ...
    console.log('Launching browser TANPA proxy settings...');
    return await puppeteer.launch(launchOptions);
  }

  // 🆕 METHOD UNTUK QUICK SWITCH PROXY
  shouldRetryWithNewProxy(error) {
    return error.message.includes('net::ERR_TIMED_OUT') ||
  ...
  // 🆕 OPTIMASI: Gunakan step multiplier dari proxy data
  async executeAllSteps(page, sessionId, config, stepMultiplier) {
    const steps = [
      {
        name: 'STEP_3',
        action: async () => {
      ...
      },
      {
        ...
        action: async () => {
          ...
          const adClicked = await this.clickGoogleAdsAndReturn(page, sessionId, config.targetUrl, 60000 * stepMultiplier);
          if (!adClicked) {
  ...
  async clickGoogleAdsAndReturn(page, sessionId, targetUrl, timeout) {
    try {
      this.log(sessionId, 'GOOGLE_ADS_START', 
        `Looking for Google ads dengan timeout ${timeout/1000} detik...`);
            ...
            if (visibleElements.length > 0) {
              ...
              if (adUrl && (adUrl.includes('google') || adUrl.includes('doubleclick'))) {
                ...
                this.log(sessionId, 'RETURN_TO_TARGET', 
                  `Returning to target website dengan timeout ${timeout/1000}s...`);
