...
class TrafficGenerator {
  ...
  async startNewSession(config) {
    ...
    this.log(sessionId, 'SESSION_INIT', 'Initializing new session dengan comprehensive proxy testing...');
    
    let proxyList = [];
    
    if (config.proxySource === 'auto') {
      this.log(sessionId, 'PROXY_AUTO', 'Mengambil proxy dengan comprehensive testing...');
      ...
      try {
        ...
        if (proxyList.length > 0) {
          ...
          this.log(sessionId, 'PROXY_AUTO_SUCCESS', 
            `Berhasil mendapatkan ${proxyList.length} proxy yang terverifikasi`);
          
          // Log detail proxies
          proxyList.forEach((proxy, index) => {
            const proxyData = this.proxyScraper.activeProxies.get(proxy);
            const info = proxyData ? `(IP: ${proxyData.ip}, Speed: ${proxyData.speed}ms)` : '';
      ...
      this.log(sessionId, 'PROXY_MANUAL', `Testing ${config.proxyList.length} manual proxies...`);
      
      const testResults = await this.proxyScraper.bulkTestProxies(config.proxyList);
      ...
      if (workingProxies.length > 0) {
        ...
        this.log(sessionId, 'PROXY_MANUAL_SUCCESS', 
          `${workingProxies.length}/${config.proxyList.length} proxy manual berhasil diverifikasi`);
      } else {
    ...
    this.log(sessionId, 'SESSION_STARTED', 
      ...
      ` menggunakan ${proxyList.length} verified proxies` +
  ...
  async fallbackToBackupProxies(config, sessionId) {
    ...
    if (config.backupProxies && config.backupProxies.length > 0) {
      // Test backup proxies
      const testResults = await this.proxyScraper.bulkTestProxies(config.backupProxies);
