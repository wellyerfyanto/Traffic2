...
class TrafficGenerator {
  constructor() {
    ...
    // 🆕 KONFIGURASI ITERASI BARU
    this.iterationConfig = {
      minIterations: 2,    // Minimal 2x kunjungan
      maxIterations: 5,    // Maksimal 5x kunjungan  
      adPageMultiplier: 2.0, // Perlambat 2x untuk halaman beriklan
      baseInteractionTime: 1.5 // Tambah 50% waktu dasar
    };
  ...
  // 🆕 METHOD: Deteksi apakah halaman mengandung iklan
  async detectAdPage(page, sessionId) {
    try {
      const hasAds = await page.evaluate(() => {
        // Cek elemen iklan Google
  ...
  // 🆕 METHOD: Reading pause yang diperpanjang
  async extendedReadingPause(sessionId, contentComplexity, multiplier = 1.0) {
    ...
    switch(contentComplexity) {
      case 'simple': baseMin = 5000; baseMax = 12000; break;
      case 'complex': baseMin = 15000; baseMax = 40000; break;
      case 'medium': default: baseMin = 8000; baseMax = 25000;
    }
  ...
  // 🆕 METHOD: Scroll yang lebih lambat
        ...
        await new Promise((resolve) => {
          ...
          const getScrollDelay = () => {
            ...
            if (patternData.patternType === 'FAST_READER') {
              ...
              baseDelay = 400 + Math.random() * 500;
            } else if (patternData.patternType === 'DETAILED_READER') {
              baseDelay = 300 + Math.random() * 400;
            } else {
          ...
          const scrollStep = () => {
            const baseSpeed = 60; // Lebih lambat dari sebelumnya
            ...
            if (patternData.patternType === 'FAST_READER') {
              ...
              speedMultiplier = 0.4 + (Math.random() * 0.2);
            } else {
  ...
  // 🆕 METHOD: Cari URL untuk halaman berikutnya
        ...
        anchors
          .filter(a => {
            ...
            return href && 
                   ...
                   !a.closest('footer') &&
                   !a.closest('nav') &&
                   new URL(href).hostname === new URL(currentUrl).hostname; // Same domain only
          })
          .map(a => ({
            href: a.href,
  ...
  // 🆕 METHOD: Interaksi iklan yang lebih komprehensif
  async comprehensiveAdInteraction(page, sessionId, multiplier = 1.0) {
    try {
      ...
      // Multiple scroll melalui area iklan
      for (let i = 0; i < 3; i++) {
        ...
        // Hover pada berbagai elemen iklan
        await page.evaluate(() => {
          const adElements = document.querySelectorAll('[class*="ad"], [id*="ad"], .adsbygoogle, ins');
          adElements.forEach((ad, index) => {
            if (index < 3) { // Hover maksimal 3 elemen
  ...
  async executeAllSteps(page, sessionId, config) {
    ...
    // 🆕 TENTUKAN JUMLAH ITERASI ACAK (2-5x)
    ...
    for (let iteration = 1; iteration <= iterationCount; iteration++) {
      ...
      // 🆕 STEPS DENGAN WAKU YANG DIPERPANJANG
      const extendedSteps = [
        {
        ...
        },
        {
          ...
          action: async () => {
            ...
            const pauseMultiplier = pattern.patternType === 'DETAILED_READER' ? 2.0 : 1.5;
            const extendedMultiplier = pauseMultiplier * interactionMultiplier;
            await this.sessionDelay(sessionId, 15000 * extendedMultiplier, 35000 * extendedMultiplier);
          },
          timeout: 60000
        },
        {
      ...
      for (const [index, step] of extendedSteps.entries()) {
        try {
          ...
          if (index < extendedSteps.length - 1) {
            await this.sessionDelay(sessionId, 5000 * interactionMultiplier, 15000 * interactionMultiplier);
          }
      ...
      if (iteration < iterationCount) {
        ...
        // Jeda antar iterasi
        await this.sessionDelay(sessionId, 10000 * interactionMultiplier, 25000 * interactionMultiplier);
      }
  ...
  async finalAdEngagement(page, sessionId) {
    try {
      ...
      await page.evaluate(() => {
        const adElements = document.querySelectorAll('[class*="ad"], [id*="ad"], .adsbygoogle');
        adElements.forEach((ad, index) => {
          if (index < 5) { // Hover maksimal 5 elemen terakhir
            const event = new MouseEvent('mouseover', {
              view: window,
              bubbles: true,
              cancelable: true
