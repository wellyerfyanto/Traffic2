...
class TrafficGenerator {
  constructor() {
    ...
    this.iterationConfig = {
      ...
      adLoadingWait: 15000, // ✅ Increased untuk ads loading
      networkIdleWait: 10000 // ✅ NEW: Tunggu network idle
    };
    ...
    this.adDomains = [
      ...
      // ✅ NEW: Additional ad networks
      'adsystem', 'advertising.com', 'yieldmanager', 'quantserve',
      'zedo.com', 'adtech', 'adition.com', 'smartadserver',
      'serving-sys.com', 'atemda.com', 'adbrn.com',
      // ✅ NEW: Content delivery untuk stylesheet & images
      'cloudflare.com', 'ajax.googleapis.com', 'fonts.googleapis.com',
      'fonts.gstatic.com', 'bootstrapcdn.com', 'cloudfront.net',
      'amazonaws.com', 'jsdelivr.net', 'unpkg.com', 'cdnjs.com',
      'jquery.com', 'w.org', 'wp.com', 'wordpress.com'
    ];
  ...
  async launchBrowser(config, proxyUrl) {
    ...
    const args = [
      ...
      // ✅ KEEP cache arguments for performance
      '--aggressive-cache-discard',
      '--disable-application-cache',
      '--disable-offline-load-stale-cache',
      '--disk-cache-size=0'
      // ❌ REMOVED: '--disable-extensions', '--disable-component-extensions-with-background-pages'
    ];
  ...
  async setupNoInterception(page, sessionId) {
    ...
    page.on('response', (response) => {
      ...
      // Log untuk stylesheet & images yang penting
      if ((resourceType === 'stylesheet' || resourceType === 'image') && status === 200) {
        if (url.includes('google') || url.includes('doubleclick') || url.includes('gstatic')) {
          this.log(sessionId, 'AD_ASSET_LOADED', 
            `🎨 ${resourceType.toUpperCase()} untuk ads: ${url.substring(0, 60)}...`);
        }
      }
    });
    ...
    page.on('console', msg => {
      if (msg.type() === 'error') {
        const text = msg.text();
        if (text.includes('ads') || text.includes('google') || text.includes('ad')) {
          this.log(sessionId, 'CONSOLE_ERROR', `🚨 ${text.substring(0, 100)}`);
        }
      }
    });
  ...
  // 🎯 ENHANCED NAVIGATION - Better Ad Loading
  async navigateWithRetry(page, url, sessionId, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // ✅ GUNAKAN networkidle2 untuk tunggu semua resource termasuk ads
        await page.goto(url, { 
          waitUntil: ['domcontentloaded', 'networkidle2'], // ✅ Changed to networkidle2
          timeout: 60000 // ✅ Increased timeout
        });
        
        // ✅ EXTENDED WAIT UNTUK ADS LOADING COMPLETE
        this.log(sessionId, 'AD_NETWORK_WAIT', '⏳ Menunggu EXTENDED untuk ad networks complete loading...');
        
        // Tunggu initial ad loading
        await this.waitForTimeout(8000);
        
        // ✅ SCROLL UNTUK TRIGGER LAZY LOAD ADS
        ...
        if (attempt < maxRetries) {
          ...
          // ❌ JANGAN throw error, continue anyway
          return false;
        }
      }
    }
  ...
  // 🆕 NEW METHOD: Trigger Lazy Load Ads
  async triggerLazyLoadAds(page, sessionId) {
    try {
      this.log(sessionId, 'LAZY_LOAD_TRIGGER', '🔄 Trigger lazy load ads dengan scroll...');
      
      // Scroll positions untuk trigger lazy load
      const scrollPositions = [200, 500, 800, 1200, 1600, 2000];
      ...
      for (const position of scrollPositions) {
        ...
        // Check for new ads after each scroll
        const newAds = await page.evaluate(() => {
          const ads = document.querySelectorAll('.adsbygoogle, [data-ad-load="lazy"]');
          return ads.length;
        });
        
        if (newAds > 0) {
          this.log(sessionId, 'LAZY_ADS_LOADED', `🎯 ${newAds} lazy ads loaded setelah scroll ke ${position}px`);
        }
      }
      ...
      this.log(sessionId, 'LAZY_LOAD_COMPLETE', '✅ Lazy load triggering selesai');
    } catch (error) {
      this.log(sessionId, 'LAZY_LOAD_ERROR', `Error trigger lazy load: ${error.message}`);
    }
  }

  // 🆕 NEW METHOD: Check Resource Loading Status
    ...
    try {
      const resources = await page.evaluate(() => {
        ...
        const adResources = resources.filter(resource => 
          ...
          resource.name.includes('gstatic')
        );
        ...
        const stylesheets = Array.from(document.styleSheets).map(sheet => {
          try {
            ...
            return { href: 'blocked', rules: 0 };
          }
        }).filter(sheet => sheet.href !== 'blocked');
        ...
        return {
          ...
          adResourceDetails: adResources.map(r => ({
            name: r.name.substring(0, 80),
            duration: r.duration,
            size: r.transferSize || 0,
            type: r.initiatorType
          })),
          stylesheetDetails: stylesheets.map(s => ({
            href: s.href ? s.href.substring(0, 60) : 'inline',
            rules: s.rules
          }))
        };
      });
      ...
      this.log(sessionId, 'RESOURCE_STATUS', 
        `📊 Resources: ${resources.totalResources} total, ${resources.adResources} ad resources, ${resources.stylesheetsLoaded} stylesheets, ${resources.imagesLoaded}/${resources.totalImages} images`);
      
      if (resources.adResources > 0) {
        resources.adResourceDetails.forEach(ad => {
          this.log(sessionId, 'AD_RESOURCE_DETAIL', 
            `🔍 ${ad.type}: ${ad.name} - Time: ${ad.duration.toFixed(2)}ms`);
        });
      }
      
      if (resources.stylesheetsLoaded > 0) {
        this.log(sessionId, 'STYLESHEET_STATUS', 
          `🎨 ${resources.stylesheetsLoaded} stylesheets loaded dengan ${resources.stylesheetDetails.reduce((sum, s) => sum + s.rules, 0)} total rules`);
      }
      
      return resources;
    } catch (error) {
      this.log(sessionId, 'RESOURCE_CHECK_ERROR', `Error check resources: ${error.message}`);
      return null;
    }
  }

  // 🎯 ENHANCED AD DETECTION - Better Selectors
  async detectAdPage(page, sessionId) {
    try {
      const adData = await page.evaluate(() => {
        // ✅ EXTENDED AD SELECTORS - Lebih comprehensive
        const adSelectors = [
          // Google Adsense
          '.adsbygoogle', 'ins.adsbygoogle', 'iframe[src*="googleads"]', 
          'iframe[src*="doubleclick"]', 'div[id*="google_ads"]', '[data-google-query-id]',
          'a[href*="googleadservices.com"]', 'iframe[id*="google_ads_iframe"]',
          
          // General Ad Containers
          '[id*="ad-container"]', '.ad-unit', '.ad-container', '[data-ad-status]', 
          '.ad-slot', '[data-ad-client]', '[class*="ad-" i]', '[id*="ad-" i]', 
          '.ad-banner', '.ad-wrapper', '.ad-placeholder', '.google-ad', '.ads-wrapper',
          '.advertising', '[data-ad-load]', '[data-ad-slot]', '.lazy-ad',
          '.ad-placement', '.ad-surface', '[data-ad-type]', '.ad-holder', 
          '.ad-frame', '.ad-content', '[data-ad-format]', '[data-ad-layout]',
          '[data-ad-width]', '[data-ad-height]', '.AdSense', '.advertisment',
          '.sponsored-content', '.text-ad', '.native-ad', '.display-ad',
          'div[data-ad-unit-path]', '[data-ad-slot]', '.ads-area', '.ad-block',
          '.ad-space', '.advert', '.promo-box', '.sponsored',
          
          // ✅ NEW: Additional ad patterns
          '[id*="banner"]', '.banner-ad', '[class*="banner"]',
          '.advertisement', '[id*="adsense"]', '.adsense',
          '.ad-item', '.adlink', '.ad-area', '.adbox',
          '.ad-heading', '.ad-text', '.ad-image',
          // ✅ NEW: Social media ads
          '[id*="facebook"]', '.fb-ad', '[data-fb-placement]',
          '.twitter-ad', '.promoted-tweet',
          // ✅ NEW: Video ads
          '.video-ad', '.ad-video', '[data-video-ad]'
        ];
          ...
          try {
            ...
            for (const element of elements) {
              ...
              const isInViewport = rect.top >= 0 && 
                                  rect.left >= 0 && 
                                  rect.bottom <= (viewportHeight + 200) && // ✅ Increased buffer
                                  rect.right <= (viewportWidth + 200);
              
              const isVisible = rect.width > 0 && rect.height > 0;
              const isLargeAd = (rect.width * rect.height) > 20000; // ✅ Lower threshold
              const isViewableArea = (rect.width * rect.height) > 8000; // ✅ Lower threshold
              const isGoogleAd = selector.includes('google') || 
                                element.outerHTML?.includes('google') ||
              ...
              if (isVisible) {
                ...
                if (isInViewport && isViewableArea) {
                  ...
                  if (isInViewport && rect.width >= 250 && rect.height >= 200) { // ✅ Lower thresholds
                  ...
                  detectedAds.push({
                    ...
                    potentialImpression: (rect.width >= 250 && rect.height >= 200),
                    inViewport: true,
                    tagName: element.tagName,
                    hasStyles: !!element.style && Object.keys(element.style).length > 0,
                    hasContent: element.innerHTML.length > 10
                  });
            ...
            // Continue dengan selector berikutnya
            continue;
          }
        }
        
        // ✅ CHECK FOR IFRAMES YANG MUNGKIN BERISI ADS
        try {
            ...
            if (src.includes('google') || src.includes('doubleclick') || src.includes('ads')) {
              ...
              if (isVisible) {
                ...
                if (isInViewport) {
                  ...
                  detectedAds.push({
                    selector: 'iframe[ad-src]',
                    position: { top: rect.top, left: rect.left },
                    size: { width: rect.width, height: rect.height },
                    viewable: true,
                    area: rect.width * rect.height,
                    isLarge: (rect.width * rect.height) > 20000,
                    isGoogleAd: true,
                    elementTop: iframe.offsetTop,
                    potentialImpression: true,
                    inViewport: true,
                    tagName: 'IFRAME',
                    hasStyles: true,
                    hasContent: true,
      ...
      // ✅ IMPROVED LOGGING UNTUK BETTER VISIBILITY
      if (adData.viewableAds > 0) {
        this.log(sessionId, 'AD_VIEWABLE_DETECTED', 
          `🎯 ${adData.viewableAds} VIEWABLE ADS DITEMUKAN! (${adData.googleAdsCount} Google Ads, ${adData.potentialImpressions} potential impressions)`);
        
        // Log detail untuk ads yang terdeteksi
        adData.detectedAds.filter(ad => ad.viewable).forEach((ad, index) => {
          this.log(sessionId, 'AD_DETAIL', 
            `📊 Ad ${index + 1}: ${ad.tagName} ${ad.selector} - Size: ${ad.size.width}x${ad.size.height} - Area: ${ad.area}px - Styles: ${ad.hasStyles} - Content: ${ad.hasContent}`);
        });
      ...
      this.log(sessionId, 'NO_ADS_DETECTED', '❌ Tidak ada iklan yang terdeteksi dengan selector current');
      return {
        hasAds: false,
        viewableCount: 0,
        largeViewableCount: 0,
  ...
  async executeSession(sessionId, config) {
    ...
    try {
      ...
      try {
        ...
        // ❌ JANGAN throw, continue dengan execution steps
        this.log(sessionId, 'CONTINUE_EXECUTION', 'Melanjutkan dengan execution steps meskipun navigasi gagal...');
        await this.executeAllSteps(page, sessionId, config);
      }
      ...
      if (currentProxy) {
        ...
        this.log(sessionId, 'PROXY_REMOVED', `Proxy gagal dihapus: ${currentProxy.url}`);
      }
      throw error;
    } finally {
      if (browser) {
  ...
  log(sessionId, step, message) {
    ...
    if (step.includes('ERROR') || step.includes('FAILED')) {
      ...
      console.log('📖', logMessage);
    } else if (step.includes('RESOURCE') || step.includes('STYLESHEET')) {
      console.log('📊', logMessage);
    } else {
      console.log('✅', logMessage);
    }
  ...
  async executeAllSteps(page, sessionId, config) {
    ...
    this.log(sessionId, 'ITERATION_SETUP', `Akan melakukan ${iterationCount} iterasi dengan NO RESOURCE BLOCKING`);
