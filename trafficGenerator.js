...
class TrafficGenerator {
  constructor() {
    ...
    this.userAgents = {
      ...
      mobile: [
        // Android - Chrome Mobile (Paling Populer)
  ...
  // 🆕 METHOD: Dapatkan random user agent dengan device type
  getRandomUserAgent(deviceType) {
    const agents = this.userAgents[deviceType] || this.userAgents.desktop;
    const selectedAgent = agents[Math.floor(Math.random() * agents.length)];
    
    // Log user agent yang dipilih
    const browserInfo = this.extractBrowserInfo(selectedAgent);
    return selectedAgent;
  }

  // 🆕 METHOD: Extract info browser dari user agent string
  ...
  // 🆕 METHOD: Tentukan pattern session berdasarkan ID (ganjil/genap & perkalian)
  getSessionPattern(sessionId) {
    ...
    // Ganjil: Pattern cepat/sedang, Genap: Pattern lambat/detail
    let patternKey;
    if (isEven) {
    ...
    // Tambahkan variasi random kecil (±20%)
  ...
  // 🆕 METHOD: Delay dengan multiplier session
  ...
  // 🆕 METHOD: Pause membaca dengan variasi berdasarkan pattern
  async readingPause(sessionId, contentComplexity = 'medium') {
    const pattern = this.getSessionPattern(sessionId);
    
    // Base time berdasarkan kompleksitas konten
    let baseMin, baseMax;
    switch(contentComplexity) {
      case 'simple':
        baseMin = 3000; // 3 detik
        baseMax = 8000; // 8 detik
        break;
      case 'complex':
        baseMin = 10000; // 10 detik
        baseMax = 25000; // 25 detik
        break;
      case 'medium':
      default:
        baseMin = 5000; // 5 detik
        baseMax = 15000; // 15 detik
    }
    ...
    this.log(sessionId, 'READING_PAUSE', 
      `Berhenti membaca ${contentComplexity} content: ${Math.round(delay/1000)} detik [${pattern.patternType}]`
    );
  ...
  async executeSession(sessionId, config) {
    ...
    try {
      ...
      // 🆕 PILIH USER AGENT RANDOM DENGAN DEVICE TYPE
  ...
  async launchBrowser(config, proxyUrl) {
    ...
    // 🆕 STEALTH OPTIONS: Tambahkan argumen untuk menghindari deteksi
    args.push(
      '--disable-dev-shm-usage',
      '--disable-accelerated-2d-canvas',
      '--no-first-run',
  ...
  // 🆕 METHOD: Scroll seperti manusia dengan variasi pattern session
      ...
      await page.evaluate(async (patternData) => {
        await new Promise((resolve) => {
          ...
          const scrollInterval = setInterval(() => {
            // Variasi kecepatan scroll berdasarkan pattern
            const baseSpeed = 80;
            ...
            // PAUSE MEMBACA: Berhenti acak untuk membaca
  ...
  // 🆕 METHOD: Analisis kompleksitas konten halaman
  ...
  async executeAllSteps(page, sessionId, config) {
    ...
    this.log(sessionId, 'SESSION_PATTERN', 
      `Pattern: ${pattern.description} (${pattern.patternType}) - Multiplier: ${pattern.finalMultiplier.toFixed(2)} - ${pattern.isEven ? 'GENAP' : 'GANJIL'}`
    );

    // Analisis kompleksitas konten
    ...
    const steps = [
      {
        ...
        successMessage: 'Membaca konten utama selesai',
        timeout: 60000
      },
      {
        ...
        successMessage: 'Scroll pertama selesai',
        timeout: 120000
      },
      {
        ...
        successMessage: 'Pause membaca selesai',
        timeout: 30000
      },
      {
        name: 'INTERACTIVE_PAUSE',
        action: async () => {
          this.log(sessionId, 'INTERACTIVE_PAUSE', 'Pause interaktif (seperti menggerakkan mouse)...');
          await page.mouse.move(
            Math.random() * 300, 
            Math.random() * 300,
            { steps: 10 }
          );
          await this.sessionDelay(sessionId, 2000, 6000);
        },
        successMessage: 'Pause interaktif selesai',
        timeout: 10000
      },
      {
        ...
        successMessage: 'Proses klik link selesai',
        timeout: 30000
      },
      {
        ...
        successMessage: 'Membaca halaman baru selesai',
        timeout: 60000
      },
      {
        ...
        successMessage: 'Scroll halaman baru selesai',
        timeout: 120000
      },
      {
        name: 'FINAL_READING_PAUSE',
        action: async () => {
          this.log(sessionId, 'FINAL_READING_PAUSE', 'Pause membaca akhir...');
          await this.readingPause(sessionId, contentComplexity);
        },
        successMessage: 'Pause membaca akhir selesai',
        timeout: 30000
      }
    ];
    ...
    for (const [index, step] of steps.entries()) {
      try {
        ...
        this.log(sessionId, `${step.name}_COMPLETE`, 
          `${step.successMessage} (${Math.round(stepDuration/1000)} detik)`
        );
  ...
  async clickRandomLink(page, sessionId) {
    try {
      ...
      if (links.length > 0) {
        ...
        this.log(sessionId, 'SELECTED_LINK', 
          `Memilih link: "${selectedLink.text}" [${pattern.patternType}]`
        );
  ...
  log(sessionId, step, message) {
    ...
    } else if (step.includes('PATTERN') || step.includes('READING')) {
