...
app.post('/api/start-session', async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            sessionId,
            message: 'Session started successfully dengan stealth plugin',
            details: {
                sessionId,
                targetUrl: config.targetUrl,
    ...
    try {
        ...
        res.json({
            ...
            system: {
                ...
                features: 'Advanced Ads Detection + Stealth + Viewable Optimization'
            },
            currentStatus: {
...
app.get('/api/test-puppeteer', async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            message: 'Puppeteer Extra + Stealth is working correctly - Railway Ready',
            version: version,
            type: 'Extra + Stealth Version - Railway Optimized',
...
// Start Server
app.listen(PORT, () => {
    console.log(`\n🚀 Advanced Traffic Bot Server v4.3.0 - Railway Optimized`);
    console.log(`🌐 http://localhost:${PORT}`);
    console.log(`⚡ Puppeteer: 24.15.0+ EXTRA + STEALTH VERSION`);
    console.log(`🎯 Focus: Google Ads Detection & Viewable Optimization`);
    ...
    setTimeout(async () => {
        try {
            ...
            const version = await browser.version();
            await browser.close();
            console.log(`✅ Puppeteer Ready: ${version}`);
        } catch (error) {
